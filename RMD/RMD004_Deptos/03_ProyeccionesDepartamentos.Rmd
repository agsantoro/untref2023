...
El objetivo de este documento es presentar una forma programática de procesamiento de las proyecciones de poblacion por departamento 2010-2025 publicadas por el INDEC, usando el software estadístico R [@R-base] con sus librerías dplyr [@dplyr], shiny [@Shiny], openxlsx y readxl [@readxl].
...
shinyApp(
  ...
  ui = fluidPage(
    selectInput("ano", "Seleccionar año:",
                choices = unique(proyecciones_depto$ano)),
    selectInput("juri", "Seleccionar jurisdicción:",
                choices = unique(proyecciones_depto$juri_nombre)),
    selectInput("sexo", "Seleccionar sexo:",
                choices = unique(proyecciones_depto$sexo_nombre)),
    highchartOutput("plot")
  ),
  
  server = function(input, output) {
    output$plot = renderHighchart({
      data_plot = proyecciones_depto[proyecciones_depto$ano==input$ano &
                                     proyecciones_depto$juri_nombre==input$juri &
                                     proyecciones_depto$sexo_nombre==input$sexo ,] %>% arrange(desc(poblacion)) %>% as.data.frame()
      
      hchart(data_plot, "bar", hcaes(x = departamento_nombre, y = poblacion)) %>%
        hc_title(text = paste0("Población de ",input$juri,". Año ",input$ano)) %>%
